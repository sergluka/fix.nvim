local Document = require("fix.document")

local M = {}

---@param opts FixOpts
---@param bufnr number
---@param ns_id number
---@param lineno number
---@param field Field
function M.annotate_field(opts, bufnr, ns_id, lineno, field)
    if field.tag_text and opts.annotate.tag.enabled then
        local tag_text = opts.annotate.tag.formatter(field)
        if tag_text then
            field._extmark_tag_id = vim.api.nvim_buf_set_extmark(bufnr, ns_id, lineno, field.tag_end, {
                id = field._extmark_tag_id,
                virt_text = { tag_text },
                virt_text_pos = "inline",
            })
        end
    end

    if field.value_text and opts.annotate.value.enabled then
        local value_text = opts.annotate.value.formatter(field)
        if value_text then
            field._extmark_value_id = vim.api.nvim_buf_set_extmark(bufnr, ns_id, lineno, field.value_end, {
                id = field._extmark_value_id,
                virt_text = { value_text },
                virt_text_pos = "inline",
            })
        end
    end
end

---@param opts FixOpts
---@param bufnr number
---@param ns_id number
---@param message Message
function M.annotate_title(opts, bufnr, ns_id, message)
    local line_shift
    if opts.annotate.message.position == "above" then
        line_shift = 0
    else
        line_shift = 1
    end
    message.extmark_title_id = vim.api.nvim_buf_set_extmark(bufnr, ns_id, message.lineno + line_shift, 0, {
        id = message.extmark_title_id,
        virt_lines = opts.annotate.message.formatter(message),
        virt_lines_above = true,
    })
end

---@param opts FixOpts
---@param bufnr number
---@param ns_id number
function M.annotate_all(opts, bufnr, ns_id)
    if not vim.api.nvim_buf_is_loaded(bufnr) then
        return
    end

    vim.api.nvim_buf_clear_namespace(bufnr, ns_id, 0, -1)

    Document.iter_messages(bufnr, function(message)
        M.annotate_message(opts, bufnr, ns_id, message)
    end)
end

---@param opts FixOpts
---@param bufnr number
---@param ns_id number
---@param message Message
function M.annotate_message(opts, bufnr, ns_id, message)
    if opts.annotate.message.enabled then
        local ok, result = pcall(M.annotate_title, opts, bufnr, ns_id, message)
        if not ok then
            vim.notify_once("failed to annotate message: " .. result, vim.log.levels.ERROR)
        end
    end
    if opts.annotate.tag.enabled or opts.annotate.value.enabled then
        for _, field in pairs(message:fields()) do
            M.annotate_field(opts, bufnr, ns_id, message.lineno, field)
        end
    end
end

return M
