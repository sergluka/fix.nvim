...
- `lua/fix/init.lua` — `setup()` deep-merges user opts over `default_settings`, registers filetype patterns/extensions, and wires an autocmd group `fix-decorate` that calls `Render.attach` on `FileType`/`BufWinEnter` and `Render.refresh_viewport` on scroll/resize. Also holds `opts_initial` so toggling "all off -> all on" restores the user's per-scope enabled flags. Re-running `setup()` drops the render cache and re-renders all open buffers. Exposes `cache_clear()` for `:FIX cache clear` and `use_dictionary(path)` for `:FIX dictionary <path>`. New options blocks: `cache.persist` and `render`.
...
- `lua/fix/dictionary.lua` — `Dictionary.load(version)` resolves either a registered custom dictionary or bundled `xml/<version>/Base/{Fields,Enums}.xml`, parses it via `xml2lua`, and caches by dictionary source. Custom dictionaries are registered with `Dictionary.register(path)` and may be QuickFIX single-file XML or FIX Repository directories. `FIXT.1.1` transparently resolves to `FIX.5.0SP2` unless explicitly registered. Messages are looked up via tag-35 enum (`dict:message(value)`).
